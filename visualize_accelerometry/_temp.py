...
from bokeh.models import TextInput, Arrow, Button, LabelSet, Div, NormalHead, DatePicker, Slider, ColumnDataSource, RangeTool, HoverTool,  BoxSelectTool, Select, DatetimeTickFormatter, Span, Label
...
def make_plot(srs, colsource, data_annot_chairstand, data_annot_3m_walk, data_annot_6min_walk, title):
    ...
    lblset_chairstand = LabelSet(x='start_time', y=200, x_units='data', y_units='screen', text="chairstand",
                                 source=data_annot_chairstand)
    lblset_3m_walk = LabelSet(x='start_time', y=200, x_units='data', y_units='screen', text="3m_walk",
                                 source=data_annot_3m_walk)
    lblset_6min_walk = LabelSet(x='start_time', y=200, x_units='data', y_units='screen', text="6min_walk",
...
def update_annotations():
    ...
    data_annot_chairstand.data.update(bp.ColumnDataSource(pdf_displayed_annotations.loc[pdf_displayed_annotations['artifact'] == 'chairstand']
                        [['start_epoch', 'end_epoch', 'start_time', 'end_time', 'artifact']]).data)
    data_annot_6min_walk.data.update(bp.ColumnDataSource(pdf_displayed_annotations.loc[pdf_displayed_annotations['artifact'] == '6min_walk']
        [['start_epoch', 'end_epoch', 'start_time', 'end_time', 'artifact']]).data)
    data_annot_3m_walk.data.update(bp.ColumnDataSource(pdf_displayed_annotations.loc[pdf_displayed_annotations['artifact'] == '3m_walk']
        [['start_epoch', 'end_epoch', 'start_time', 'end_time', 'artifact']]).data)
    ...
    #
    # pdf_timedwalk_annotation = pdf_annotations_to_display.loc[pdf_annotations_to_display['artifact'] == '6min_walk']
...
print(pdf_displayed_annotations)
...
layout = grid(column(row(column(row(file_picker, sizing_mode='stretch_width'),
                     ...
                     row(column(p, select)),
                     row(column(selected_data_title, selected_table, sizing_mode='stretch_both'), sizing_mode='stretch_both'),
